...
from google.adk.models.lite_llm import LiteLlm # This import is likely unused but kept for parity
    ...
    async for event in runner.run_async(user_id=USER_ID, session_id=SESSION_ID, new_message=message):
        if event.is_final_response():
            ...
            try:
                ...
                if JSON_OUTPUT_KEY in parsed and isinstance(parsed[JSON_OUTPUT_KEY], list):
                    return {JSON_OUTPUT_KEY: parsed[JSON_OUTPUT_KEY]}
                else:
                    print(f"'{JSON_OUTPUT_KEY}' key missing or not a list in response JSON")
